...
function(add_external_package_dir pkg)
	...
	if(ARGC GREATER 1)
		...
		if(EXISTS ${Package_source}/CMakeLists.txt)
			debug_message("########### used implicit version ${Package_source}")
			get_filename_component(ver ${Package_source} NAME)
			get_filename_component(Package_source ${Package_source} PATH)
			get_filename_component(pkg ${Package_source} NAME)
			debug_message("########### used implicit version ${Package_source}/${pkg}/${ver}")
		else()
			if(EXISTS ${Package_source}/Default.cmake)
				...
				foreach(item ${versions})
					if(NOT IS_DIRECTORY ${Package_source}/${item})
						list(REMOVE_ITEM versions ${item})
					else()
						if(NOT EXISTS ${Package_source}/${item}/CMakeLists.txt)
							list(REMOVE_ITEM versions ${item})
						endif()
	...
	if(pkg_found LESS 0 )
		if(EXISTS ${Package_source}/${ver}/CMakeLists.txt)
			...
			if(NOT my_found OR ForceBuild )
				...
				set(Package_dependency_file "")
				if(EXISTS ${Package_source}/${ver}/Depends.cmake)
					set(Package_dependency_file ${Package_source}/${ver}/Depends.cmake)
				else()
					if(EXISTS ${Package_source}/Depends.cmake)
						set(Package_dependency_file ${Package_source}/Depends.cmake)
					endif()
				endif()
				if(Package_dependency_file)
					set(Package_current_dependencies "")
					include(${Package_dependency_file})
