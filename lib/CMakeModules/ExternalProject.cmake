  ...
  elseif(url)
    ...
    if(IS_DIRECTORY "${url}")
      ...
      if("${url}" MATCHES "^[a-z]+://")
        ...
        string(REGEX MATCH "^.*\\.(zip|tgz|tar\\.gz|tar\\.bz2|tar)" url_trimmed "${url}")
        string(REGEX MATCH "[^/]*$" fname "${url_trimmed}")
        if(NOT "${fname}" MATCHES "\\.(zip|tar|tgz|tar\\.gz|tar\\.bz2)$")
          message(FATAL_ERROR "Could not extract tarball filename  -->${fname}<--  -->${url_trimmed}<-- from url:\n  ${url}")
        endif()
        set(file ${download_dir}/${fname})
        get_property(timeout TARGET ${name} PROPERTY _EP_TIMEOUT)
        _ep_write_downloadfile_script("${srcstamp_dir}/download-${name}.cmake" "${url}" "${file}" "${timeout}")
